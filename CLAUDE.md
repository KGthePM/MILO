...
Components live in **feature subdirectories**: `components/movies/`, `tv/`, `podcasts/`, `books/`, `friends/`, `timeline/`, `settings/`, `recommendations/`, `onboarding/`, and `shared/`.
...
**Sign-in backdrop**: `frontend/src/utils/neonHorizon.js` draws the synthwave grid + starfield on a `<canvas>` (wrapper: `components/shared/NeonHorizon.jsx`), used by `AuthGate`, `AppLockGate`, and the `/landing` hero. It is the app's only `requestAnimationFrame`/canvas code. It replaced a CSS perspective grid that iOS smeared into a colour wash — WebKit rasterises a 3D-transformed layer once into a fixed backing store and then lets the perspective magnify that bitmap, so the near hairlines were a stretched cache. **Do not reintroduce a CSS-3D-transformed fine-line texture.** The DPR cap of 2 and the 30fps native idle cap are deliberate (see commit `bf98bb8` on WKWebView main-thread starvation); lane density is derived from viewport width so it survives rotation. `WARP_MS` is exported and shared with AuthGate's sign-in hand-off timer — don't fork the constant. The warp is a 2.4s sequence (charge → launch → cruise → exit flash, `warpCurve()`), skippable by tap (`WARP_SKIP_MS`, the engine's `skip()`). `utils/warpEggs.js` flies canvas line-art easter eggs past the periphery: six per warp, plus an opening burst of three as the page loads and an idle drift-by every 5–9s after that where the caller passes `eggs="idle"` (AuthGate and the `/landing` hero only — never the reel, where cards are being read, or the calm lock curtain).

**Onboarding**: `components/onboarding/`. `OnboardingReel.jsx` is a five-card intro, mounted in `GatedApp` and portaled at `z-[60]`, above the `z-50` modals. It shows when `onboarding.introSeen` is false. It sits over the same `NeonHorizon` canvas as sign-in, so the warp lands straight in it. Its cards are solid panels, not `.glass`, because blur over the moving canvas recomposites every frame. It ends on deep links: `/settings?tab=<id>` (SettingsPage honours only tab ids that exist in the current build) and `/<type>?add=1` (`useAddFromQuery` opens the page's Add modal, then strips the param). `DiscoveryHint.jsx` is a one-time inline callout. It sits in the normal layout, not floated over it, and is held back while the reel is open. There are four hint ids: `recs-intro` and `rec-feedback` (EnhancedRecommendations), `title-search` (TitleSearch), and `assistant-fab` (MiloAssistantFab's bubble, shown after 3 or more watched items, with 3 bounded pulses). State lives per device in `milo.userPrefs.v1` under `onboarding: { introSeen, hints }`. Use the helpers (`setIntroSeen`, `markHintSeen`, `resetHints`), which re-read storage before writing, and `useOnboarding()` for live reads. Any other writer to prefs must merge onto a fresh `loadUserPrefs()` and not hold a stale copy; `AppearanceSection` does this. Replay and reset live in Settings → Appearance → Guidance.
